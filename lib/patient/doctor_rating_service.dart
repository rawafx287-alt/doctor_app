...
double doctorRatingAverageFromData(Map<String, dynamic> data) {
  ...
  final parsedAvg = double.tryParse(avg?.toString().trim() ?? '');
  if (parsedAvg != null) return parsedAvg.clamp(0, 5);
  ...
  final parsedSum = double.tryParse(sum?.toString().trim() ?? '');
  if (parsedSum != null && count > 0) {
    return (parsedSum / count).clamp(0, 5);
  }
  return 0;
}
...
int doctorRatingCountFromData(Map<String, dynamic> data) {
  ...
  final parsed = int.tryParse(c?.toString().trim() ?? '');
  if (parsed != null) return parsed.clamp(0, 1 << 30);
  return 0;
}
...
Future<void> submitDoctorRating({
  ...
  await FirebaseFirestore.instance.runTransaction((tx) async {
    ...
    final oldSumRaw = d[DoctorRatingFirestore.ratingSum];
    final oldSum = (oldSumRaw is num)
        ? oldSumRaw.toDouble()
        : (double.tryParse(oldSumRaw?.toString().trim() ?? '') ?? 0.0);
