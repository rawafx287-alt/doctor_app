...
const Color _kSecPatCellMuted = Color(0xFF90A4AE);
...
class _MasterCalendarScreenState extends State<MasterCalendarScreen> {
  ...
  /// Same layout rules as [PatientAvailableDaysList._patientDayCell]; past days
  /// stay tappable (no [IgnorePointer]).
  Widget _secretaryPatientStyleCell({
    ...
    if (isSelected) {
      ...
      textColor = _kSecPatPastSlate;
    } else if (isOutside) {
      textColor = _kSecPatCellMuted;
    } else if (!closedOrFull) {
      textColor = Colors.white;
    } else {
      textColor = Colors.white;
    ...
    final isOpenGreen = !isPast && !isOutside && !closedOrFull && !isSelected;
    final isClosedRed = !isPast && !isOutside && closedOrFull && !isSelected;
